...
const FriendRequests = () => {
  ...
  return (
    <div className={`h-full w-[65%] flex flex-col gap-2 rounded-3xl bg-gradient-to-r from-blue-600 to-violet-600 dark:bg-purple-950  p-4`}>
...
const Request: React.FC<reqProp> = ({req}) =>{
    ...
    <div className='flex gap-3 items-center'>
      ...
      <h1 className='text-lg w-max h-5'>{req.sender.username}</h1>
        <span className='text-sm h-5 w-max text-gray-500'>send you friend request</span>
      </div>
    </div>
    <div className='flex gap-3'>
        <button 
        className={`bg-black/80 ${isAccepted ? "text-green-500" : "text-sky-500"} h-7 w-max px-2 rounded-lg`}
        onClick={async()=>{
            const res = await axios.put(`${import.meta.env.VITE_BASE_URL}/friend/accept`,{
        ...
        }}>{isAccepted ? 'Accepted' : 'Accept'}</button>
        <button className='bg-black/80 text-orange-500 h-7 w-max px-2 rounded-lg'>Reject</button>
    </div>
</div>
}
